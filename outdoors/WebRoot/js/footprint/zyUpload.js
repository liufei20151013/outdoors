	...
	$.fn.zyUpload = function(options,param){
		...
		return this.each(function(){
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
					// 创建带有拖动的html
					html += '<form id="uploadForm" action="'+$("#imagePostUrl").val()+'" method="post" enctype="multipart/form-data">';
		            ...
		            html += '					<input class="upload_btn" style="hight=34px;wight=94px;" type="submit" value="上传图片">';
		            html += '				</div>';
		            html += '			</div>';
				    ...
				    html += '							<img class="upload_image" src="http://localhost:9000/outdoors/images/footprint/add_img.png" style="width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width)" />';
				    html += '						</div>';
				    html += '					</a>';
				    html += '				</div>';
					html += '			</div>';
